...
\postertitle{Demo: Forecasting a Caf\'e's Daily Ingredient Demand With
...
\textbf{Coffee beans, milk, water, cocoa powder, sugar, salt.}
\end{itemize}
\end{uhhbox}
...
\begin{itemize}
\item Lightweight Forecasting Pipeline
...
\vspace{2mm}
\centering
\includegraphics[width=0.93\linewidth]{dis_architecture.png}
\end{uhhbox}
...
\$ ./timeseries.py -i milk -fu "2026-07-14" -p 1.0 \quad{\rmfamily\itshape (undamped trend!)}}\\
\vspace{4mm}

\begin{itemize}
\item All four smoothing parameters ($\alpha,\beta,\gamma,\phi$) are
      command line flags.
\item Forecasting window defined by $\texttt{-ff}$ ("forecast from") 
      and $\texttt{-fu}$ ("forecast until")
%\item Every Plot appears immediately.
\end{itemize}
\end{uhhbox}
...
{The model forecasts the \textbf{last two observed months}
using only the history before the window.}
\end{uhhbox}
...
{Two-month forecast \textbf{beyond the data}: the answer to
